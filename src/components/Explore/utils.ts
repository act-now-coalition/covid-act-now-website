...
export function getChartUrl(sharedComponentId: string, region?: Region) {
...
export function getLocationLabel(location: Region) {
  ...
  } else if (location.regionType === RegionType.MSA) {
    return location.fullName;
  } else {
...
export function getAutocompleteLocations(locationFips: string) {
  ...
  const sortedMetros = sortBy(regions.metroAreas, location => location.name);
